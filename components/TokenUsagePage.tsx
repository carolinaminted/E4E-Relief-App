...
const TokenUsagePage: React.FC<TokenUsagePageProps> = ({ navigate, currentUser }) => {
  ...
  const handleRefresh = useCallback(() => {
    ...
    // Simulate a brief delay for the animation to be visible, as the local data fetch is instant.
    setTimeout(() => {
      fetchData();
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto w-full relative min-h-[calc(100vh-100px)]">
        ...
        <div className="relative flex justify-center items-center mb-6 py-1">
            ...
            <button 
                ...
                className="absolute right-0 bg-[#004b8d] hover:bg-[#005ca0] text-white font-semibold p-2 rounded-md text-sm transition-colors duration-200 flex items-center justify-center border border-[#005ca0] disabled:opacity-50 disabled:cursor-not-allowed"
